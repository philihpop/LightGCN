"""
Custom Dataset Implementation for LightGCN Recommendation System
"""
import os
...
class AmazonBeauty(BasicDataset):
    """
    Dataset type for pytorch
    Include graph information for LightGCN
    """
    def __init__(self, path="../data/beauty"):
        # Path to data files
        cprint("Loading [CustomDataset]")
        self.path = path
        ...
        metadata_file = join(path, 'cleaned_metadata.csv')
        ...
        self.train_unique_users = np.unique(train_user_ids)
        self.test_unique_users = np.unique(test_user_ids)
        
        # Determine number of users and items
        self.n_user = max(np.max(train_user_ids), np.max(test_user_ids)) + 1
        self.m_item = max(np.max(train_item_ids), np.max(test_item_ids)) + 1
        ...
        self.train_data_size = len(train_user_ids)
        self.test_data_size = len(test_user_ids)
        
        # Print statistics
        print(f"{self.train_data_size} interactions for training")
        print(f"{self.test_data_size} interactions for testing")
        print(f"CustomDataset Sparsity: {(self.train_data_size + self.test_data_size) / self.n_user / self.m_item}")
        ...
        # Calculate user and item degrees (for normalization)
        ...
        print("CustomDataset is ready to go")
    ...
    def getSparseGraph(self):
        ...
        if self.Graph is None:
            try:
                ...
                print(f"Adjacency matrix generated in {end-s}s")
    ...
    def __build_test(self):
        """
        Build the test set as a dictionary
    ...
    def getUserPosItems(self, users):
        ...
        users: list of users
        return: list of positive item lists
        """
        posItems = []
        for user in users:
    ...
    def get_submission_recommendations(self, model, submission_file, top_k=10):
        """
        ...
        with torch.no_grad():
            for user in user_ids:
                if user >= self.n_users:
                    ...
                    recommendations = np.random.choice(range(self.m_items), size=top_k, replace=False)
                else:
                    # Get user embedding
                    ...
                    user_pos_items = self.getUserPosItems([user])[0]
                ...
                results.append({'ID': sub_df['ID'][sub_df['user_id'] == user].values[0],
                               'user_id': user,
                               'item_id': rec_str})
